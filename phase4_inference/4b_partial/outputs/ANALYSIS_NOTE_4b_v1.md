...
abstract: |
  ...
  A 10% data validation using a random subsample of the collision data
  confirms the analysis framework functions correctly on real data, with
  all nuisance parameter pulls below 2$\sigma$ and the primary NN score
  goodness-of-fit $p$-value of 0.209. A VBF category deficit of 35%
  is observed, consistent with statistical fluctuations in the
  low-statistics 10% subsample. Full observed results will be added
  after unblinding.
...
This analysis note presents the expected results obtained on Asimov pseudo-data together with a 10% data validation using a random subsample of the collision data. The 10% validation confirms the analysis framework functions correctly on real data, with all nuisance parameter pulls below 2$\sigma$ and acceptable goodness-of-fit for the primary NN approach. Full observed results on the complete collision dataset will be added after unblinding approval.
...
**Proceed to Phase 4c (full data)** with the expectation that the VBF deficit will be reduced with full statistics. The VBF data/MC ratio should be monitored closely in the full dataset.


# Conclusions {#sec:conclusions}

This analysis note presents the expected results and 10% data validation for a measurement of the Higgs boson signal strength modifier $\mu$ in the $H\to\tau\tau$ decay channel, performed in the $\mu\tau_\mathrm{h}$ final state using 11.5 fb$^{-1}$ of CMS Open Data at $\sqrt{s} = 8$ TeV. Three fitting approaches are compared using a simultaneous binned profile likelihood fit across Baseline and VBF event categories.
...
The expected uncertainty of $\sigma(\mu) = 1.28$ is approximately 4.7 times larger than the published CMS combined result of $\pm 0.27$ [@CMS:2014nkk], consistent with the reduced scope of this single-channel, single-run-period analysis. This measurement can distinguish predictions differing by approximately $\Delta\mu \approx 2.6$ at $2\sigma$ significance.

The 10% data validation confirms the analysis framework functions correctly on real data. The primary NN approach yields $\hat{\mu} = -3.73 \pm 2.81$ (pull = $-1.69\sigma$ from SM), with a goodness-of-fit $p$-value of 0.209 and all nuisance parameter pulls below 2$\sigma$. With extended POI bounds [-30, 30], no fit hits the boundary for any approach. The Baseline category shows healthy data/MC agreement (ratio = 0.95), while the VBF category exhibits a 35% deficit (data/MC = 0.65) attributable to statistical fluctuations in the low-statistics 10% subsample. The per-category fit confirms that the Baseline alone is consistent with the SM ($\hat{\mu} = -0.25 \pm 1.57$, pull = $-0.79\sigma$).

All validation tests pass their respective criteria: the NN shows no overtraining (KS $p > 0.05$ for both signal and background), signal injection tests recover injected values with pulls below 0.01, and the fit framework converges correctly on both Asimov and 10% real data. The analysis is ready for unblinding pending human approval.
