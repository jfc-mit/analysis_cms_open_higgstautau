"""
Phase 4b Step 3: Fit 10% data for signal strength.

For each approach (m_vis, nn_score, m_col):
- Load the pyhf workspace from Phase 4a
- Replace Asimov observations with 10% data (scaled x10)
- Fit for mu
- Compute NP pulls, GoF, significance
- Compare to Phase 4a expected results
"""
import logging
import json
import numpy as np
...
def load_workspace_with_data(approach):
    """Load 4a workspace and replace Asimov data with 10% real data (x10)."""
    # Load workspace
    with open(P4A / f"workspace_{approach}.json") as f:
        spec = json.load(f)

    # Load 10% data histograms
    with open(OUT / "data_histograms_10pct.json") as f:
        data_hists = json.load(f)

    # Replace observations with scaled 10% data
    for obs in spec["observations"]:
        channel_name = obs["name"]
        # Parse approach and category from channel name
        # Format: "{approach}_{category}"
        parts = channel_name.split("_", 1)
        if len(parts) == 2:
            obs_approach, cat = parts[0], parts[1]
            # Handle nn_score which has underscore
            if obs_approach == "nn":
        ...
        if obs_approach in data_hists["data_histograms"]:
            ...
            if cat_data:
                scaled_data = cat_data["data_10pct_scaled"]
                log.info(
                    f"  {channel_name}: replacing Asimov with 10% data x10 "
                    f"(sum={sum(scaled_data):.0f})"
                )
                obs["data"] = scaled_data

    return spec


def fit_data(approach):
    """Fit 10% data for one approach."""
    log.info(f"\n=== Fitting {approach} on 10% data ===")

    spec = load_workspace_with_data(approach)
    ...
    p_value = float(np.mean(np.array(clean_chi2s) >= chi2_val)) if clean_chi2s else None
    log.info(
        f"  GoF p-value = {p_value:.4f} "
        ...
        if cls_scan:
            ...
            for i in range(len(poi_arr) - 1):
                if obs_arr[i] >= 0.05 >= obs_arr[i + 1]:
                    ...
                    obs_limit_95 = float(poi_arr[i] + frac * (poi_arr[i + 1] - poi_arr[i]))
                if exp_med_arr[i] >= 0.05 >= exp_med_arr[i + 1]:
                    frac = (0.05 - exp_med_arr[i]) / (exp_med_arr[i + 1] - exp_med_arr[i])
                    exp_limit_95 = float(poi_arr[i] + frac * (poi_arr[i + 1] - poi_arr[i]))
...
def main():
    # Load 4a expected results for comparison
    ...
    # Comparison summary
    log.info("\n=== 10% Data vs Expected Comparison ===")
    log.info(
    ...
    for approach in APPROACHES:
        ...
        if r["gof_pvalue"] is not None and r["gof_pvalue"] < 0.05:
            log.warning(f"  {approach}: GoF p-value = {r['gof_pvalue']:.4f} (< 0.05)")
            any_flag = True
        for np_name, np_info in r["np_pulls"].items():
            if abs(np_info["pull"]) > 2.0:
                log.warning(
                    f"  {approach}: NP {np_name} pull = {np_info['pull']:.2f} (> 2 sigma)"
                )
                any_flag = True

    if not any_flag:
        log.info("  No flags raised. All results consistent with expected.")
