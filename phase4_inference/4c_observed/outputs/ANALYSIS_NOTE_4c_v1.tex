...
uncertainty of 1.25 due to data constraints on the nuisance parameters.
...
techniques for separating signal from background shape effects. Two
nuisance parameters are pulled beyond \(2\sigma\) (\(W\)+jets VBF
normalization and jet energy scale shape), both attributable to the
known VBF category data/MC deficit of 31\%.
\end{abstract}
...
2\(\sigma\). \\
4c-v1 & 2026-03-24 & Phase 4c: Full data results. Added Section 11 (Full
Data Results) with observed signal strengths, NP pulls, GoF,
per-category fits, impact ranking, and post-fit data/MC comparisons for
all three approaches. Primary result: NN score \(\mu = 0.63 \pm 1.08\)
(0.61\(\sigma\) observed significance). Three-way comparison table
(expected / 10\% / full) shows NN score recovering toward SM with full
statistics. VBF deficit persists at 31\% (data/MC = 0.69) but is
absorbed by normalization NPs. Two NPs pulled beyond 2\(\sigma\):
norm\_wjets\_vbf (\(-2.16\sigma\)) and shape\_jes (\(-2.02\sigma\)).
Updated abstract and conclusions with final physics result. \\
\end{tabular}
\end{table}
}
...
with \(\sigma(\mu) = 1.25\).}\label{tbl:asimov-results}
\begin{tabular}{@{}
...
\begin{figure}[htbp]
\centering
\pandocbounded{\includegraphics[height=0.35\textheight,width=\linewidth,keepaspectratio,alt={Distribution of \textbackslash chi\^{}2 values from toy fits (blue histogram) compared to the observed \textbackslash chi\^{}2 = 34.4 from the 10\% data NN score fit (red vertical line). The p-value of 0.209 (fraction of toys exceeding the observed \textbackslash chi\^{}2) indicates that the post-fit model provides an acceptable description of the 10\% data. This represents a meaningful test --- unlike the Asimov GoF in Figure fig.~ which is p = 1.0 by construction --- because the real data introduces genuine statistical fluctuations.}]{../../4b_partial/outputs/figures/gof_toys_10pct.pdf}}
...
The analysis proceeded to Phase 4c (full data) following human gate
approval.
...
both the expected (Asimov, Phase 4a) and 10\% data validation (Phase 4b)
results.
...
pattern was already observed in the 10\% validation (Phase 4b) and
reflects known limitations of the MC modeling in the VBF-enriched phase
space for this open data sample. The Baseline deficit is consistent with
the known overestimate from the QCD same-sign estimation method (Section~\ref{sec:bkg-summary}), while the VBF deficit indicates a
systematic normalization issue that persists from the 10\% subsample to
the full dataset.
...
Figures~\ref{fig:full-prefit-nn-baseline} through
Figure~\ref{fig:full-prefit-mcol-vbf}.

\needspace{0.4\textheight}
\begin{figure}[htbp]
\centering
\pandocbounded{\includegraphics[height=0.35\textheight,width=\linewidth,keepaspectratio,alt={Pre-fit data/MC comparison for the NN score distribution in the Baseline category using the full dataset. The stacked MC prediction includes Z\textbackslash to\textbackslash tau\textbackslash tau (dominant), Z\textbackslash to\textbackslash ell\textbackslash ell, t\textbackslash bar\{t\}, W+jets, and QCD multijet backgrounds, with the ggH and VBF signal components. The 5\% data deficit is visible across the full NN score range, with the ratio panel showing systematic deviations from unity.}]{figures/data_mc_prefit_nn_score_baseline.pdf}}
\caption{Pre-fit data/MC comparison for the NN score distribution in the
...
unity.}\label{fig:full-prefit-nn-baseline}
\end{figure}

\needspace{0.4\textheight}
\begin{figure}[htbp]
\centering
\pandocbounded{\includegraphics[height=0.35\textheight,width=\linewidth,keepaspectratio,alt={Pre-fit data/MC comparison for the NN score distribution in the VBF category using the full dataset. The 31\% data deficit is clearly visible across all NN score bins. The deficit is uniform in shape, indicating a normalization issue rather than a shape mismodeling. The reduced statistics lead to larger bin-by-bin fluctuations in the ratio panel.}]{figures/data_mc_prefit_nn_score_vbf.pdf}}
\caption{Pre-fit data/MC comparison for the NN score distribution in the
...
panel.}\label{fig:full-prefit-nn-vbf}
\end{figure}

\needspace{0.4\textheight}
\begin{figure}[htbp]
\centering
\pandocbounded{\includegraphics[height=0.35\textheight,width=\linewidth,keepaspectratio,alt={Pre-fit data/MC comparison for the visible mass distribution in the Baseline category. The Z\textbackslash to\textbackslash tau\textbackslash tau peak near 60-80 GeV is clearly visible. The 5\% data deficit is consistent with the QCD overestimation identified in Section sec.~. The shape agreement is good across the full mass range.}]{figures/data_mc_prefit_mvis_baseline.pdf}}
\caption{Pre-fit data/MC comparison for the visible mass distribution in
...
range.}\label{fig:full-prefit-mvis-baseline}
\end{figure}

\needspace{0.4\textheight}
\begin{figure}[htbp]
\centering
\pandocbounded{\includegraphics[height=0.35\textheight,width=\linewidth,keepaspectratio,alt={Pre-fit data/MC comparison for the visible mass distribution in the VBF category. The 31\% deficit is present across the full mass range, most prominently in the Z peak region. The reduced event count leads to large statistical fluctuations.}]{figures/data_mc_prefit_mvis_vbf.pdf}}
\caption{Pre-fit data/MC comparison for the visible mass distribution in
...
fluctuations.}\label{fig:full-prefit-mvis-vbf}
\end{figure}

\needspace{0.4\textheight}
\begin{figure}[htbp]
\centering
\pandocbounded{\includegraphics[height=0.35\textheight,width=\linewidth,keepaspectratio,alt={Pre-fit data/MC comparison for the collinear mass distribution in the Baseline category. The broader distribution compared to visible mass reflects the unphysical collinear solution fallback. The 5\% deficit is consistent with the other approaches.}]{figures/data_mc_prefit_mcol_baseline.pdf}}
\caption{Pre-fit data/MC comparison for the collinear mass distribution
...
\begin{figure}[htbp]
\centering
\pandocbounded{\includegraphics[height=0.35\textheight,width=\linewidth,keepaspectratio,alt={Pre-fit data/MC comparison for the collinear mass distribution in the VBF category. The 31\% deficit and broader distribution due to unphysical solutions are both visible. The collinear mass shows the largest bin-to-bin fluctuations among the three approaches.}]{figures/data_mc_prefit_mcol_vbf.pdf}}
\caption{Pre-fit data/MC comparison for the collinear mass distribution
...
approaches.}\label{fig:full-prefit-mcol-vbf}
\end{figure}
...
observed 95\% CL upper limit on \(\mu\) is 2.85 (expected: 2.45 from
Phase 4a with the updated workspace).

The result is consistent with both the Standard Model expectation
(\(\mu = 1.0\), within \(0.34\sigma\)) and with the Phase 4a expected
uncertainty (\(\sigma_\mathrm{expected} = 1.25\) vs
...
a large \(Z\to\tau\tau\) background. These approaches are sensitive to
normalization mismodeling because signal and background have overlapping
shapes. The NN score approach mitigates this by concentrating signal
sensitivity in high-score bins where the signal-to-background ratio is
favorable.
...
analysis phases. The expected results are from Asimov pseudo-data (Phase
4a), the 10\% data from the validation subsample (Phase 4b), and the
full data from the complete collision dataset (Phase
4c).}\label{tbl:three-way-comparison}
\begin{tabular}{@{}
...
\item
  ...
  10\% fluctuation washed out with more data, as anticipated in the
  Phase 4b interpretation.
\item
...
\caption{Nuisance parameter pulls from the NN score full data fit. Two
NPs are pulled beyond 2\(\sigma\): the \(W\)+jets VBF normalization and
the JES shape systematic. Both are attributable to the VBF category
data/MC deficit.}\label{tbl:full-np-pulls}\tabularnewline
\toprule\noalign{}
NP & Pull (\(\sigma\)) & Post-fit unc. & Flag \\
\midrule\noalign{}
\bottomrule\noalign{}
norm\_wjets\_vbf & \(-2.16\) & 0.59 & \textgreater{} 2\(\sigma\) \\
shape\_jes & \(-2.02\) & 0.56 & \textgreater{} 2\(\sigma\) \\
norm\_wjets\_baseline & \(+1.50\) & 0.54 & \\
norm\_qcd\_vbf & \(-1.48\) & 0.68 & \\
shape\_met\_uncl & \(-1.14\) & 0.60 & \\
norm\_qcd\_baseline & \(+1.12\) & 0.34 & \\
norm\_missing\_bkg & \(-0.94\) & 0.75 & \\
norm\_ztt & \(-0.79\) & 0.40 & \\
btag\_eff & \(-0.72\) & 0.74 & \\
norm\_ttbar & \(-0.72\) & 0.74 & \\
tau\_id\_eff & \(-0.72\) & 0.65 & \\
lumi & \(-0.60\) & 0.68 & \\
trigger\_eff & \(-0.58\) & 0.73 & \\
muon\_id\_iso & \(-0.44\) & 0.78 & \\
shape\_tes & \(+0.42\) & 0.30 & \\
shape\_mes & \(-0.13\) & 0.63 & \\
Theory NPs & \textless{} 0.01 & \textasciitilde1.0 & unconstrained \\
\end{longtable}

Two nuisance parameters are pulled beyond 2\(\sigma\):

\begin{itemize}
\item
  \textbf{norm\_wjets\_vbf (\(-2.16\sigma\)):} The \(W\)+jets
  ...
  significant background there (approximately 30\% of the VBF
  prediction).
\item
  \textbf{shape\_jes (\(-2.02\sigma\)):} The JES shape systematic is
...
These pulls are not alarming for an open data analysis with known MC
limitations. The theory NPs are essentially unconstrained (post-fit
uncertainty approximately 1.0), as expected for normalization-type
systematics that are poorly constrained by this dataset.

\needspace{0.4\textheight}
\begin{figure}[htbp]
\centering
\pandocbounded{\includegraphics[height=0.35\textheight,width=\linewidth,keepaspectratio,alt={Nuisance parameter best-fit values and post-fit uncertainties from the NN score full data fit. The black points show the best-fit values with error bars indicating the post-fit uncertainties. Two parameters (norm\_wjets\_vbf and shape\_jes) are pulled beyond 2\textbackslash sigma (indicated by the yellow band boundary), reflecting the VBF category data/MC deficit. All other parameters are within 2\textbackslash sigma. The shape systematics (TES, MES, JES, MET unclustered) show significant constraint from the data (post-fit uncertainties well below 1.0).}]{figures/np_pulls_full.pdf}}
\caption{Nuisance parameter best-fit values and post-fit uncertainties
from the NN score full data fit. The black points show the best-fit
values with error bars indicating the post-fit uncertainties. Two
parameters (norm\_wjets\_vbf and shape\_jes) are pulled beyond
\(2\sigma\) (indicated by the yellow band boundary), reflecting the VBF
category data/MC deficit. All other parameters are within \(2\sigma\).
The shape systematics (TES, MES, JES, MET unclustered) show significant
constraint from the data (post-fit uncertainties well below
1.0).}\label{fig:full-np-pulls}
\end{figure}
...
strength \(\mu\) from the NN score full data fit. The three shape
systematics (MES, JES, MET unclustered) dominate the uncertainty budget
with a combined impact of approximately
0.7.}\label{tbl:full-impact}
\begin{tabular}{@{}
...
\midrule\noalign{}
\bottomrule\noalign{}
1 & MES (muon energy scale) & \(+0.344\) & \(-0.531\) & 0.438 \\
2 & JES (jet energy scale) & \(-0.703\) & \(-0.125\) & 0.414 \\
3 & MET unclustered & \(+0.000\) & \(+0.672\) & 0.336 \\
4 & \(Z\to\tau\tau\) norm & \(+0.155\) & \(-0.076\) & 0.115 \\
5 & \(W\)+jets norm (VBF) & \(-0.142\) & \(-0.044\) & 0.093 \\
6 & QCD norm (baseline) & \(-0.019\) & \(+0.139\) & 0.079 \\
7 & QCD norm (VBF) & \(-0.097\) & \(-0.009\) & 0.053 \\
8 & Luminosity & \(-0.077\) & \(+0.022\) & 0.049 \\
9 & b-tag efficiency & \(-0.077\) & \(+0.014\) & 0.045 \\
10 & \(t\bar{t}\) norm & \(-0.077\) & \(+0.014\) & 0.045 \\
\end{tabular}
\end{table}

The three shape systematics (MES, JES, MET unclustered) dominate the
uncertainty budget, contributing a combined impact of approximately
\(\sqrt{0.438^2 + 0.414^2 + 0.336^2} \approx 0.69\) to \(\Delta\mu\).
...
The goodness-of-fit test is performed using 200 toy pseudo-experiments
for each approach. The results are summarized in
Table~\ref{tbl:full-gof}.

\vspace{1em}
\begin{table}[htbp]
\centering
\small
\caption{Goodness-of-fit results on the full dataset. The negative ndf
arises because the model has more parameters (71) than data bins
(approximately 45-50), which is standard for heavily parameterized
HistFactory models. The toy-based \(p\)-values of 0.00 reflect this
over-parameterization rather than genuine
mismodeling.}\label{tbl:full-gof}
\begin{tabular}{@{}
>{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1408}}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.3239}}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.0704}}
...
\midrule\noalign{}
\bottomrule\noalign{}
NN score & 33.1 & \(-22\) & 0.00 & 200 \\
\(m_\mathrm{vis}\) & 39.2 & \(-22\) & 0.00 & 200 \\
\(m_\mathrm{col}\) & 39.2 & \(-22\) & 0.00 & 200 \\
\end{tabular}
\end{table}

The effective number of degrees of freedom is negative (\(-22\)) because
the model has more parameters (\(n_\mathrm{par} \approx 71\): 21 NPs +
approximately 50 Barlow-Beeston gammas + 1 POI) than data bins
(\(n_\mathrm{bins} \approx 45\)-50 across both categories). This is a
known feature of heavily parameterized HistFactory models with many
normalization and shape nuisance parameters. The negative ndf means the
\(\chi^2\)/ndf metric is not meaningful, and the toy-based GoF
\(p\)-value of 0.00 reflects the fact that the observed \(\chi^2\) (33.1
for NN score) exceeds all toy \(\chi^2\) values.

For context, the post-fit data/MC agreement is visually good in the NN
score approach (Figures~\ref{fig:full-postfit-nn-baseline} and
Figure~\ref{fig:full-postfit-nn-vbf}), and the fit converges without
numerical issues. The GoF limitation is a consequence of the model
parameterization, not of the data description quality.

The GoF toy distributions are shown in Figures~\ref{fig:full-gof-nn} through Figure~\ref{fig:full-gof-mcol}.

\needspace{0.4\textheight}
\begin{figure}[htbp]
\centering
\pandocbounded{\includegraphics[height=0.35\textheight,width=\linewidth,keepaspectratio,alt={Distribution of \textbackslash chi\^{}2 values from 200 toy fits (blue histogram) compared to the observed \textbackslash chi\^{}2 = 33.1 from the full data NN score fit (red vertical line). The observed value exceeds all toy values, yielding a p-value of 0.00. This reflects the over-parameterized model (negative ndf) rather than genuine data/model disagreement.}]{figures/gof_toys_full_nn_score.pdf}}
\caption{Distribution of \(\chi^2\) values from 200 toy fits (blue
...
values, yielding a \(p\)-value of 0.00. This reflects the
over-parameterized model (negative ndf) rather than genuine data/model
disagreement.}\label{fig:full-gof-nn}
\end{figure}

\needspace{0.4\textheight}
\begin{figure}[htbp]
\centering
\pandocbounded{\includegraphics[height=0.35\textheight,width=\linewidth,keepaspectratio,alt={GoF toy distribution for the m\_\textbackslash mathrm\{vis\} approach on full data. The observed \textbackslash chi\^{}2 = 39.2 exceeds all toys (p = 0.00). The larger observed \textbackslash chi\^{}2 compared to the NN score reflects the poorer fit of the m\_\textbackslash mathrm\{vis\} approach to the data normalization.}]{figures/gof_toys_full_mvis.pdf}}
\caption{GoF toy distribution for the \(m_\mathrm{vis}\) approach on
...
normalization.}\label{fig:full-gof-mvis}
\end{figure}

\needspace{0.4\textheight}
\begin{figure}[htbp]
\centering
\pandocbounded{\includegraphics[height=0.35\textheight,width=\linewidth,keepaspectratio,alt={GoF toy distribution for the m\_\textbackslash mathrm\{col\} approach on full data. The observed \textbackslash chi\^{}2 = 39.2 (identical to m\_\textbackslash mathrm\{vis\}) exceeds all toys. The similarity between the m\_\textbackslash mathrm\{vis\} and m\_\textbackslash mathrm\{col\} GoF values suggests both approaches are similarly affected by the normalization deficit.}]{figures/gof_toys_full_mcol.pdf}}
\caption{GoF toy distribution for the \(m_\mathrm{col}\) approach on
...
deficit.}\label{fig:full-gof-mcol}
\end{figure}
...
\begin{figure}[htbp]
\centering
\pandocbounded{\includegraphics[height=0.35\textheight,width=\linewidth,keepaspectratio,alt={Post-fit data/MC comparison for the NN score distribution in the Baseline category. After the profile likelihood fit, the predicted yields are adjusted through the nuisance parameter pulls, improving the data/MC agreement. The ratio panel shows the post-fit data/prediction ratio, which is closer to unity than the pre-fit comparison in Figure fig.~.}]{figures/data_mc_postfit_nn_score_baseline.pdf}}
\caption{Post-fit data/MC comparison for the NN score distribution in
...
ratio, which is closer to unity than the pre-fit comparison in Figure~\ref{fig:full-prefit-nn-baseline}.}\label{fig:full-postfit-nn-baseline}
\end{figure}

\needspace{0.4\textheight}
\begin{figure}[htbp]
\centering
\pandocbounded{\includegraphics[height=0.35\textheight,width=\linewidth,keepaspectratio,alt={Post-fit data/MC comparison for the NN score distribution in the VBF category. The fit absorbs the 31\% pre-fit deficit through the normalization nuisance parameters (W+jets VBF pulled to -2.16\textbackslash sigma, QCD VBF pulled to -1.48\textbackslash sigma), producing improved but not perfect data/MC agreement. The residual shape differences reflect the limitations of using normalization-only adjustments for a potentially shape-dependent effect.}]{figures/data_mc_postfit_nn_score_vbf.pdf}}
\caption{Post-fit data/MC comparison for the NN score distribution in
...
\(-2.16\sigma\), QCD VBF pulled to \(-1.48\sigma\)), producing improved
but not perfect data/MC agreement. The residual shape differences
reflect the limitations of using normalization-only adjustments for a
potentially shape-dependent effect.}\label{fig:full-postfit-nn-vbf}
\end{figure}
...
\begin{figure}[htbp]
\centering
\pandocbounded{\includegraphics[height=0.35\textheight,width=\linewidth,keepaspectratio,alt={Post-fit data/MC comparison for the visible mass distribution in the Baseline category. The large negative signal strength (\textbackslash hat\{\textbackslash mu\} = -6.70) pulls the signal contribution negative, effectively adding events to improve the data/MC agreement. This demonstrates why the m\_\textbackslash mathrm\{vis\} approach is vulnerable to normalization mismodeling.}]{figures/data_mc_postfit_mvis_baseline.pdf}}
\caption{Post-fit data/MC comparison for the visible mass distribution
...
normalization mismodeling.}\label{fig:full-postfit-mvis-baseline}
\end{figure}

\needspace{0.4\textheight}
\begin{figure}[htbp]
\centering
\pandocbounded{\includegraphics[height=0.35\textheight,width=\linewidth,keepaspectratio,alt={Post-fit data/MC comparison for the visible mass distribution in the VBF category. The fit adjustment is substantial, reflecting the combination of the 31\% data deficit and the limited discriminating power of the visible mass in the VBF category.}]{figures/data_mc_postfit_mvis_vbf.pdf}}
\caption{Post-fit data/MC comparison for the visible mass distribution
...
category.}\label{fig:full-postfit-mvis-vbf}
\end{figure}
...
\begin{figure}[htbp]
\centering
\pandocbounded{\includegraphics[height=0.35\textheight,width=\linewidth,keepaspectratio,alt={Post-fit data/MC comparison for the collinear mass distribution in the Baseline category. Similar to the m\_\textbackslash mathrm\{vis\} approach, the large negative \textbackslash hat\{\textbackslash mu\} compensates for the normalization deficit. The broader collinear mass distribution smears any localized features.}]{figures/data_mc_postfit_mcol_baseline.pdf}}
\caption{Post-fit data/MC comparison for the collinear mass distribution
...
features.}\label{fig:full-postfit-mcol-baseline}
\end{figure}

\needspace{0.4\textheight}
\begin{figure}[htbp]
\centering
\pandocbounded{\includegraphics[height=0.35\textheight,width=\linewidth,keepaspectratio,alt={Post-fit data/MC comparison for the collinear mass distribution in the VBF category. The fit adjustment is the most extreme among the three approaches, consistent with the largest negative signal strength (\textbackslash hat\{\textbackslash mu\} = -10.74). The high unphysical solution fraction in this category further dilutes the discriminating power.}]{figures/data_mc_postfit_mcol_vbf.pdf}}
\caption{Post-fit data/MC comparison for the collinear mass distribution
...
power.}\label{fig:full-postfit-mcol-vbf}
\end{figure}
...
to the expected Asimov value of 1.25, reflecting data constraints on the
nuisance parameters. The systematic uncertainty budget is dominated by
three shape systematics: muon energy scale (impact 0.44), jet energy
scale (0.41), and MET unclustered energy (0.34). The combined systematic
uncertainty is approximately 0.69, while the statistical uncertainty is
approximately 0.83, confirming that this measurement remains
statistically limited.

Two nuisance parameters are pulled beyond 2\(\sigma\) in the full data
fit: the \(W\)+jets VBF normalization (\(-2.16\sigma\)) and the JES
shape (\(-2.02\sigma\)), both attributable to the persistent 31\%
data/MC deficit in the VBF category. This deficit is a systematic
normalization issue present in both the 10\% subsample and the full
dataset, likely arising from LO generator limitations in the
VBF-enriched phase space. Despite this deficit, the NN score approach is
robust, as demonstrated by the consistent per-category results
(\(\hat{\mu}_\mathrm{Baseline} = 0.61 \pm 1.60\),
