...
\(\mu < 2.60\) for the neural network approach. Observed results will be
added after data validation.
\end{abstract}
...
pseudo-data. Observed results on collision data will be added after data
validation in a subsequent version.
...
2020}).}\label{tbl:data-samples}\tabularnewline
\toprule\noalign{}
...
tbl.~\ref{tbl:mc-samples}. The signal samples are generated with Powheg
...
control region method (Section sec.~\ref{sec:bkg-qcd}). The single top
...
(Section sec.~\ref{sec:syst-missing}).
...
simulation (Section sec.~\ref{sec:syst-znorm}).
...
tbl.~\ref{tbl:muon-selection}. The \(p_\mathrm{T}\) threshold of 20 GeV
...
to satisfy the criteria in tbl.~\ref{tbl:tau-selection}. The tau
...
mass peak region (Section sec.~\ref{sec:tauid-wp}). Tight anti-electron
...
official scale factors.}\label{tbl:tau-selection}\tabularnewline
\toprule\noalign{}
Requirement & Value \\
\midrule\noalign{}
\endfirsthead
...
mass).}\label{tbl:tauid-wp}\tabularnewline
\toprule\noalign{}
...
Figures fig.~\ref{fig:tauid-vloose} through fig.~\ref{fig:tauid-medium}.
The VLoose working point is rejected due to significantly worse data/MC
agreement (\(\chi^2\)/ndf = 4.08, Figure fig.~\ref{fig:tauid-vloose}).
The data/MC ratio of 1.12 at the Loose working point (Figure
fig.~\ref{fig:tauid-loose}) is consistent with the expected QCD multijet
...
validation, as summarized in tbl.~\ref{tbl:control-regions}.
...
charge requirements,
respectively.}\label{tbl:control-regions}\tabularnewline
\toprule\noalign{}
...
in \(W\)+jets events (Figure fig.~\ref{fig:mt-regions}) and is used to
derive the \(W\)+jets normalization scale factor (Section
sec.~\ref{sec:bkg-wjets}). The same-sign signal region provides the QCD
multijet template after subtraction of MC-predicted same-sign
backgrounds (Section sec.~\ref{sec:bkg-qcd}). The anti-isolated control
...
tbl.~\ref{tbl:cutflow-raw} for raw event counts and
tbl.~\ref{tbl:cutflow-weighted} for luminosity-weighted yields. The
...
contribution.}\label{tbl:cutflow-weighted}\tabularnewline
\toprule\noalign{}
...
contribute approximately 11,200 events to the signal region (Section
sec.~\ref{sec:bkg-qcd}).
...
tbl.~\ref{tbl:category-yields}. The VBF category achieves a VBF signal
...
data.}\label{tbl:category-yields}\tabularnewline
\toprule\noalign{}
Sample & Baseline & VBF \\
\midrule\noalign{}
\endfirsthead
...
normalization uncertainty (Section sec.~\ref{sec:syst-znorm}).
...
(\(\sigma^2_\mathrm{MC} = \sum w_i^2\)).}\label{tbl:wjets-sf}\tabularnewline
\toprule\noalign{}
Quantity & Value \\
\midrule\noalign{}
\endfirsthead
...
signal region (Section sec.~\ref{sec:syst-wjets}).
...
uncertainties.}\label{tbl:qcd-yield}\tabularnewline
\toprule\noalign{}
Quantity & Value \\
\midrule\noalign{}
\endfirsthead
...
tbl.~\ref{tbl:corrected-yields} summarizes the predicted yields from all
...
fit.}\label{tbl:corrected-yields}\tabularnewline
\toprule\noalign{}
Source & OS SR yield \\
\midrule\noalign{}
\endfirsthead
...
near 60-80 GeV (in visible mass), as shown in Figures
fig.~\ref{fig:mvis-baseline} and fig.~\ref{fig:mvis-vbf}. The poor mass
...
overtraining.}\label{tbl:nn-config}\tabularnewline
\toprule\noalign{}
Parameter & Value \\
\midrule\noalign{}
\endfirsthead
...
The NN uses 14 input features, listed in tbl.~\ref{tbl:nn-features} with
...
strategy.}\label{tbl:nn-performance}\tabularnewline
\toprule\noalign{}
...
that the NN provides meaningful signal-background discrimination (Figure
fig.~\ref{fig:nn-roc}). The modest drop from train (0.843) to test
(0.825) AUC indicates good generalization with no significant
overtraining (Figure fig.~\ref{fig:nn-overtraining}). The NN assigns
...
overtraining.}\label{tbl:nn-overtraining}\tabularnewline
\toprule\noalign{}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Overtraining check for the NN classifier. The NN score distributions are shown for signal (red) and background (blue), overlaying the training set (filled histogram) and test set (points with error bars). The good agreement between training and test distributions in both classes confirms the absence of overtraining, consistent with the KS test results in tbl.~.}]{../../../phase3_selection/outputs/figures/nn_overtraining.pdf}}
...
the individual ROC AUC values in tbl.~\ref{tbl:nn-features}, with the
...
\(N_\mathrm{jets}\) (40.68, visible in Figure
fig.~\ref{fig:njets-baseline}), \(N_\mathrm{b-jets}\) (32.35), and
\(\Delta R\) (5.16, borderline, Figure fig.~\ref{fig:delta-r-baseline}).
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={NN discriminant score distribution in the Baseline category. Background events peak at low NN scores (near 0) while signal events (shown scaled by x10) peak at high scores (near 1), demonstrating the discriminating power of the NN classifier. The QCD template is constructed by evaluating the trained NN on same-sign data events, ensuring consistent treatment of the data-driven QCD background in the fitting observable.}]{../../../phase3_selection/outputs/figures/nn_score_baseline.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={NN discriminant score distribution in the VBF category. The separation between signal and background is similar to the Baseline category, but with reduced statistics. The ratio panel shows larger fluctuations due to the limited event count in the VBF category.}]{../../../phase3_selection/outputs/figures/nn_score_vbf.pdf}}
...
set, with less overtraining.}\label{tbl:bdt-comparison}\tabularnewline
\toprule\noalign{}
...
0.820), as shown in Figure fig.~\ref{fig:bdt-roc}. The BDT shows
slightly more overtraining (train AUC 0.875 vs 0.843 for NN, Figure
fig.~\ref{fig:bdt-overtraining}), with the signal KS test \(p\)-value at
0.0007 indicating marginal overtraining. The NN is confirmed as the
better classifier for this analysis.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={ROC curve comparison between the primary NN classifier and the alternative BDT. The NN achieves AUC = 0.825 vs BDT AUC = 0.820 on the test set. The small difference confirms the NN as the preferred classifier, with the BDT result serving as a cross-check of the MVA approach.}]{../../../phase3_selection/outputs/figures/bdt_vs_nn_roc.pdf}}
...
tbl.~\ref{tbl:mcol-physical}, are higher than the strategy estimates but
...
taus and complex final states.}\label{tbl:mcol-physical}\tabularnewline
\toprule\noalign{}
...
collinear mass distributions are shown in Figures
fig.~\ref{fig:mcol-baseline} and fig.~\ref{fig:mcol-vbf}. Background
...
profile likelihood fit results are presented in Section
sec.~\ref{sec:expected-results}.}\label{tbl:approach-comparison}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Approach
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Hadronic tau p\_\textbackslash mathrm\{T\} distribution in the VBF category. The spectrum is harder than in the Baseline category due to the VBF event topology, where the Higgs boson recoils against the forward jet system. The reduced statistics lead to larger data/MC fluctuations in the ratio panel.}]{../../../phase3_selection/outputs/figures/tau_pt_vbf.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Muon p\_\textbackslash mathrm\{T\} distribution in the VBF category. The VBF signal events (visible at the x10 scale) tend to have slightly harder muon p\_\textbackslash mathrm\{T\} compared to the Baseline category, reflecting the boost from the VBF production mechanism.}]{../../../phase3_selection/outputs/figures/mu_pt_vbf.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Missing transverse energy distribution in the VBF category. The t\textbackslash bar\{t\} background contributes a larger fraction in this category compared to Baseline, due to the two-jet requirement that naturally selects t\textbackslash bar\{t\} events. The MET distribution is broader than in Baseline, consistent with the more complex event topology.}]{../../../phase3_selection/outputs/figures/met_pt_vbf.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Ranking of kinematic variables by their individual signal-vs-background separation power, quantified by the ROC AUC. The top variables (\textbackslash tau\_\textbackslash mathrm\{h\} p\_\textbackslash mathrm\{T\}, m\_\textbackslash mathrm\{vis\}, N\_\textbackslash mathrm\{jets\}, E\_\textbackslash mathrm\{T\}\^{}\textbackslash mathrm\{miss\}) all have AUC \textgreater{} 0.63, while pseudorapidity variables show minimal discrimination (AUC near 0.5). The NN combines all 14 variables to achieve a test AUC of 0.825.}]{../../../phase2_exploration/outputs/figures/separation_power.pdf}}
...
provided in tbl.~\ref{tbl:syst-summary} and the impact ranking is
presented in Section sec.~\ref{sec:syst-ranking}.
...
(Section sec.~\ref{sec:syst-znorm}) to avoid double-counting. The 3\%
...
TES the single most important systematic uncertainty (Figure
fig.~\ref{fig:syst-tes}). This dominance is expected for a template
shape fit where the signal-background discrimination depends critically
on the reconstructed mass and tau energy scales.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Ratio of the TES up (red) and down (blue) varied templates to the nominal template for Z\textbackslash to\textbackslash tau\textbackslash tau, ggH signal, and t\textbackslash bar\{t\} in the NN score Baseline channel. The TES variation produces bin-by-bin shape changes of order 5-20\%, with the largest effects in bins where the NN score distribution has steep gradients. The characteristic shape --- depletion in one region and enhancement in an adjacent region --- reflects the migration of events along the NN score axis as the tau energy is scaled.}]{figures/syst_shift_tes.pdf}}
...
0.291), as shown in Figure fig.~\ref{fig:syst-mes}. The MES is the
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Ratio of the MES up and down varied templates to the nominal template for selected processes in the NN score Baseline channel. The 1\% MES variation produces smaller shape changes than the 3\% TES variation, but the impact on \textbackslash mu is still significant because the muon p\_\textbackslash mathrm\{T\} enters directly into the discriminant variable computation.}]{figures/syst_shift_mes.pdf}}
...
\(\mu\) is \(-0.096/+0.213\) (ranked 4th, total impact 0.165, Figure
fig.~\ref{fig:syst-jes}).

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Ratio of the JES up and down varied templates to the nominal for selected processes in the NN score Baseline channel. The JES variation produces asymmetric effects due to category migration: JES up increases jet p\_\textbackslash mathrm\{T\} and moves more events into the VBF category, depleting the Baseline. The effect on the NN score distribution is less direct than TES/MES since jet p\_\textbackslash mathrm\{T\} enters the NN as a secondary input.}]{figures/syst_shift_jes.pdf}}
...
\(\mu\) is \(-0.266/+0.047\) (ranked 3rd, total impact 0.191, Figure
fig.~\ref{fig:syst-met-uncl}). The highly asymmetric impact suggests
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Ratio of the MET unclustered energy up and down varied templates to the nominal for selected processes in the NN score Baseline channel. The 10\% MET variation produces visible shape effects, particularly in bins sensitive to the MET-dependent features in the NN. The asymmetric impact pattern is characteristic of unclustered energy variations.}]{figures/syst_shift_met_uncl.pdf}}
...
tbl.~\ref{tbl:syst-completeness} compares the systematic sources
...
tbl.~\ref{tbl:impact-ranking} presents the top 15 nuisance parameters
...
to \(\Delta\mu\) (Figure fig.~\ref{fig:impact-ranking}), dominating the
...
dataset. \textbf{Observed results on collision data will be added after
data validation.}
...
both categories are shown in Figures
fig.~\ref{fig:template-mvis-baseline} through
fig.~\ref{fig:template-mcol-vbf}. The visible mass templates (Figures
fig.~\ref{fig:template-mvis-baseline} and
fig.~\ref{fig:template-mvis-vbf}) show the characteristic \(Z\) peak and
Higgs signal region. The NN score templates (Figures
fig.~\ref{fig:template-nn-baseline} and fig.~\ref{fig:template-nn-vbf})
demonstrate clear signal-background separation. The collinear mass
templates (Figures fig.~\ref{fig:template-mcol-baseline} and
fig.~\ref{fig:template-mcol-vbf}) show broader distributions due to
unphysical solutions.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Pre-fit visible mass template in the Baseline category. The stacked backgrounds (Z\textbackslash to\textbackslash tau\textbackslash tau, Z\textbackslash to\textbackslash ell\textbackslash ell, t\textbackslash bar\{t\}, W+jets, QCD) are shown with the ggH and VBF signal contributions scaled by 10 for visibility. Asimov data points (generated as the sum of all backgrounds plus SM signal) overlay the total prediction. The Z\textbackslash to\textbackslash tau\textbackslash tau background dominates with a peak near 60-80 GeV, while the Higgs signal contributes a broad excess in the 100-150 GeV region.}]{figures/template_mvis_baseline.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Pre-fit visible mass template in the VBF category. The lower statistics compared to Baseline are evident, with t\textbackslash bar\{t\} and W+jets becoming significant backgrounds alongside Z\textbackslash to\textbackslash tau\textbackslash tau. The VBF signal (blue, scaled x10) is comparable in magnitude to the ggH contamination (red, scaled x10) in this category.}]{figures/template_mvis_vbf.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Pre-fit NN discriminant score template in the Baseline category. The backgrounds peak at low NN scores while the signal peaks at high scores, demonstrating the discriminating power of the NN classifier. The clear separation between signal and background in this observable is the primary driver of the improved sensitivity of the NN approach.}]{figures/template_nn_score_baseline.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Pre-fit NN discriminant score template in the VBF category. The signal-background separation pattern is similar to Baseline, with reduced statistics. The NN exploits the VBF-specific kinematic features (forward jets, large dijet mass) to further enhance the signal discrimination in this category.}]{figures/template_nn_score_vbf.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Pre-fit collinear mass template in the Baseline category. The broader distribution compared to the visible mass reflects the inclusion of events with unphysical collinear solutions that fall back to the visible mass. For events with physical solutions, the collinear mass provides improved mass resolution, producing a sharper Z peak and better Higgs signal separation.}]{figures/template_mcol_baseline.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Pre-fit collinear mass template in the VBF category. The reduced statistics and higher unphysical solution fractions for background processes result in a broader distribution compared to the Baseline category.}]{figures/template_mcol_vbf.pdf}}
...
tbl.~\ref{tbl:asimov-results} summarizes the Asimov fit results for all
...
This confirms the pre-fit approach comparison in Figure
fig.~\ref{fig:approach-comparison}. The visible mass approach has better
...
tbl.~\ref{tbl:injection-nn}, tbl.~\ref{tbl:injection-mvis}, and
tbl.~\ref{tbl:injection-mcol} present the results for each approach.
...
unbiased signal extraction.}\label{tbl:injection-nn}\tabularnewline
\toprule\noalign{}
...
approach.}\label{tbl:injection-mvis}\tabularnewline
\toprule\noalign{}
\(\mu_\mathrm{inject}\) & \(\hat{\mu}\) & \(\sigma(\mu)\) & Pull \\
\midrule\noalign{}
\endfirsthead
...
approach.}\label{tbl:injection-mcol}\tabularnewline
\toprule\noalign{}
\(\mu_\mathrm{inject}\) & \(\hat{\mu}\) & \(\sigma(\mu)\) & Pull \\
\midrule\noalign{}
\endfirsthead
...
framework is unbiased and the signal parameterization is correct (Figure
fig.~\ref{fig:signal-injection}). The increasing \(\sigma(\mu)\) with
...
\subsection{Nuisance parameter pulls}\label{sec:val-pulls}
...
data (Figure fig.~\ref{fig:np-pulls}):
...
\subsection{Goodness of fit}\label{sec:val-gof}
...
\(\chi^2 = 0\) is always less than any toy \(\chi^2\) (Figure
fig.~\ref{fig:gof-toys}), the \(p\)-value is 1.0 for all approaches.
...
tbl.~\ref{tbl:validation-summary} provides a comprehensive summary of
...
This analysis note presents the expected results for a measurement of
the Higgs boson signal strength modifier \(\mu\) in the \(H\to\tau\tau\)
decay channel, performed in the \(\mu\tau_\mathrm{h}\) final state using
11.5 fb\(^{-1}\) of CMS Open Data at \(\sqrt{s} = 8\) TeV. Three fitting
approaches are compared using a simultaneous binned profile likelihood
fit across Baseline and VBF event categories.
...
The expected uncertainty of \(\sigma(\mu) = 1.25\) is approximately 4.6
...
\(\Delta\mu \approx 2.5\) at \(2\sigma\) significance.
...
fit framework converges correctly on Asimov data. The analysis is ready
for data validation following the unblinding protocol.

Observed results on collision data will be added in a subsequent version
